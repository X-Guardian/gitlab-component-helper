...
import { buildVersionLabels } from './componentBrowserVersionLabels';
  ...
  public registerDetailsPanelMessageHandler(
    ...
    panel.webview.onDidReceiveMessage(async message => {
      switch (message.command) {
        ...
        case 'fetchVersions': {
          try {
            if (!isCachedComponentShape(active)) {
            ...
            const cached = active;
            const versions = await this.cacheManager.fetchComponentVersions(cached);
            panel.webview.postMessage({
              command: 'versionsLoaded',
              versions,
              versionLabels: buildVersionLabels(versions, cached),
